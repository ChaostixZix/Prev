    ...
    */
    ...
    public function login(Request $request){
        ...
        if (config('app.captcha_status') && config('app.captcha_type') == 'recaptcha') {
            $messages = [
                'g-recaptcha-response.recaptcha' => __('login.error.invalid_recaptcha'),
            ];
            $request->validate([
        ...
        if (config('app.captcha_status') && config('app.captcha_type') == 'default') {
            $messages = [
                'captcha.captcha' => __('login.error.invalid_captcha'),
                'captcha.required' => __('login.error.required_captcha'),
            ];
            $request->validate([
        ...
        if ($this->attemptLogin($request)) {
            ...
            if (!empty($this->settings->maintenance) && $this->settings->maintenance->enabled && Auth()->user()->role !== 1) {
                $this->logout($request);
                return back()->with('info', __('login.info.maintenance'));
            }
            # Authentication passed...
            $values = array('user' => Auth()->user()->id, 'ip' => $request->ip(), 'os' => $agent->platform(), 'browser' => $agent->browser(), 'what' => 'login successfully', 'date' => Carbon::now($this->settings->timezone));
            DB::table('users_logs')->insert($values);
            $activity = array('activity' => Carbon::now($this->settings->timezone), 'user_agent' => $agent->platform());
            User::where('id', Auth()->user()->id)->update($activity);
            return redirect()->route('home.manage')->with('success', "You're welcome " . ucfirst(Auth()->user()->name));
        }else{
            ...
            if (DB::table('users')->where($field, $request->user)->count() > 0) {
                ...
                $values = array('user' => $user->id, 'ip' => $request->ip(), 'os' => $agent->platform(), 'browser' => $agent->browser(), 'what' => 'invalid password', 'date' => Carbon::now($this->settings->timezone));
                DB::table('users_logs')->insert($values);
            }
        }

        return back()->with('error', __('login.error.invalid_credentials'));
    }
