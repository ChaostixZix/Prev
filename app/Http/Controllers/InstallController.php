...
class InstallController extends Controller{
    ...
    public function final() {
        file_put_contents(storage_path('installed'), "Prev successfully installed on ".date("Y/m/d h:i:sa"));
...
$https = '<IfModule mod_rewrite.c>
    ...
    public function appDetailSubmit(Request $request) {
        ...
        $new_array = [];
        # Check license code
        $license = $this->getClovPrevPurchase($request->license);
        if ($license->status !== 'success'):
            if (file_exists(storage_path('app/.code'))) {
                unlink(storage_path('app/.code'));
            }
            return back()->withErrors($license->response);
            $this->log($license->response);
            else:
            $this->log('License check success. Name on license "'."BINTANG PUTRA".'". License type "'."PREMIUM".'"');
            $new_array["LICENSE_KEY"]  = $request->license;
            $new_array["LICENSE_NAME"] = "BINTANG PUTRA";
            $new_array["LICENSE_TYPE"] = "PREMIUM";
//            $new_array["LICENSE_TYPE"] = $license->response->license;
        ...
        unset($requests['_token'], $requests['force_https'], $requests['name'], $requests['email'], $requests['username'], $requests['password'], $requests['password_confirmation'], $requests['license']);
        $requests['LICENSE_KEY'] = $request->license;
