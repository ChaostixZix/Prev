        ...
        <div class="card card-inner bdrs-15 m-3 card-shadow">
              ...
              <div class="admin-pricing-box-content">
                <div class="status-info success">
                  <a href="{{request()->fullUrlWithQuery(['trans' => $file['filename']])}}" class="btn btn-primary btn-block mt-3">{{ __('Edit translation') }} <em class="icon ni ni-edit"></em></a>
                </div>
              </div>
         </div>
        @endforeach
    ...
    <div class="col-md-8">
        ...
        <form action="{{ route('admin-translation-post', 'post') }}" class="card card-shadow card-inner bdrs-20" method="post">
            ...
            <div class="form-group">
                 <input type="text" class="form-control c-textarea form-control-lg" placeholder="search" data-search />
            </div>
            <input type="hidden" name="trans" value="{{$locale}}">
