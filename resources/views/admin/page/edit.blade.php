...
@section('footerJS')
<link href="{{ url('froala/css/froala_editor.min.css') }}" rel="stylesheet" type="text/css" />
<link href="{{ url('froala/css/froala_style.css') }}" rel="stylesheet" type="text/css" />
<link href="{{ url('froala/css/plugins/table.min.css') }}" rel="stylesheet" type="text/css" />
...
<script type="text/javascript" src="{{ url('froala/js/sr.js') }}"></script>
@stop
...
<form action="{{ route('edit.post.page') }}" method="post" enctype="multipart/form-data">
	...
	<div class="form-group mt-5 mt-lg-2 content-container {{($page->type == 'external') ? 'hide' : ''}}">
	    ...
	    <div class="form-control-wrap">
	        <textarea class="form-control form-control-lg" id="edit" placeholder="enter your page content" name="content">{{ (!empty($page->settings) && !empty($page->settings->content)) ? $page->settings->content : "" }}</textarea>
	    </div>
	</div>
	<div class="form-group mt-5">
