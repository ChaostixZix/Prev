...
<div class="row">
  ...
  <div class="col-md-6">
    <div class="nk-ck mt-5">
        <canvas class="line-chart" id="linksChart"></canvas>
    </div>
  </div>
  <div class="col-md-6">
    <div class="nk-ck mt-5">
    ...
    let datepicker = $('#datepicker_input').datepicker({
        ...
        onSelect: (formatted_date, date) => {
            ...
            if(date.length > 1) {
                ...
                /* Redirect */
                redirect(`/manage/admin/stats/?start_date=${start_date}&end_date=${end_date}`);
            }
        }
    });
 </script>
 <script>
   ...
   var linksChart = {
         labels: {!! $options->linkschart['labels'] ?? "[]" !!},
         dataUnit: "{{ __('Links created') }}",
         lineTension: 0.1,
         legend: !0,
         datasets: [{
             label: "{{ __('Links') }}",
             color: "#a0aec0",
             background: "transparent",
             data: {!! !empty($options->linkschart['count']) ? $options->linkschart['count'] : '[0]' !!}
         }]
   };
